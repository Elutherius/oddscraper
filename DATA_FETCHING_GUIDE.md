...
7.  **Archive**: Copies the consolidated file plus per-sportsbook slices into `downloads/history/<timestamp>/`. These snapshots drive the "See Previous Data" table in the dashboard and guarantee past pulls remain available.
...
Each run of `run_e2e_pipeline.py` saves a timestamped folder under `downloads/history/` (e.g. `downloads/history/20260209_013045/`). Inside you'll find:

* `consolidated_odds.csv` — the master table exactly as served to the dashboard.
* One CSV per sportsbook (`draftkings.csv`, `fanduel.csv`, `polymarket.csv`, etc.) produced by filtering the consolidated output by `Source`.
* `metadata.json` — lightweight manifest used by the UI, including the recorded pull time.

How to retrieve past pulls:

1. Launch the Streamlit dashboard and click the **See Previous Data** button under the downloads section. A table appears with a "Pull Time" column plus one column per sportsbook. Click any "Download" button to grab that CSV directly in the browser.
2. Alternatively, browse the filesystem to `downloads/history/<timestamp>/` and copy any CSV you need.

This approach means new runs always refresh `consolidated_odds.csv` while keeping prior pulls preserved for auditing or backfills.
